...
testing_files = ['even-merge']
training_files = ['odd-merge']
print(lines)

# reading the csv files testings set
print('Loading data...')
for i in testing_files:
    df = pd.read_csv('.//Dataset//' + str(i) + '.csv', encoding="utf8", on_bad_lines='warn')
...
for j in training_files:
    df = pd.read_csv('.//Dataset//' + str(j) + '.csv', encoding="utf8", on_bad_lines='warn')
    for row in range(len(df)):
...
train_loader = data_utils.DataLoader(train_set, batch_size=3000, shuffle=True)  # สร้าง dataloader สำหรับ train set

# get the test set
test_set = data_utils.TensorDataset(x_testImages, y_testLabels)  # สร้าง datasets สำหรับ test
test_loader = data_utils.DataLoader(test_set, batch_size=3000, shuffle=True)  # สร้าง dataloader สำหรับ test set
